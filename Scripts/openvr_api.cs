...
namespace Valve.VRRenderingPackage
{
    ...
    public class OpenVR
    {
        private static bool openVRCached;
        private static bool openVRDllPresent;

        public static bool IsOpenVRDLLPresent()
        {
#if UNITY_STANDALONE
            if (openVRCached)
            {
                return openVRDllPresent;
            }
            openVRCached = true;

            try
            {
                OpenVRInterop.GetInitToken();
                openVRDllPresent = true;
                return openVRDllPresent; // we didnt have an exception while access interop, so we have OpenVR.dll
            }
            catch
            {
                openVRDllPresent = false;
                return openVRDllPresent;
            }
#else
            return false;
